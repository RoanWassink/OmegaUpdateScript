...
def send_show(device_dict, command):
    ...
    with ConnectHandler(**device_dict) as ssh:
        ssh.enable()
        ssh.send_config_set(command, cmd_verify=False)
        result = ssh.send_command_expect('./blup.sh', expect_string="root@albireo:~#",delay_factor=10)
...
tftp = '172.16.201.70'
commands = ['tftp -gr blup.sh 172.16.201.70', 'echo tftphost=172.16.201.70 > /tmp/serverparams.conf', 'chmod 0777 blup.sh']
